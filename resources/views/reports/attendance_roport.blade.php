            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <table id="datatable-buttons" class="table table-striped nowrap w-100 ">
                        ...
                        <tbody>
                            @foreach ($employeeAttendances as $empAttendance)
                                <tr>
                                    <td>{{ $empAttendance->employee->name }}</td>
                                    <td>{{ $empAttendance->employee->job_number }}</td>
                                    <td>{{ $empAttendance->appointment->name }}</td>
                                    <td>{{ $empAttendance->branch->name }}</td>
                                    <td>{{ $empAttendance->date }}</td>
                                    <td>{{ $empAttendance->user_name }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
...
@section('script')
    ...
    <script>
        $("#searchBtn").on('click',function(){
            window.location = "{{ route('reports.attend-report') }}?employee_id=" + $('#employees').val()+"&date="+ $('#date').val()
        });
    </script>
@endsection
